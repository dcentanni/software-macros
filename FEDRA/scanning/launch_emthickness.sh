...
if [ "$MIC" == "1" ]; then
    THEPATH="/eos/experiment/sndlhc/emulsionData/CERN/SND/RUN${RUN}/RUN${RUN}_W${WALL}_B${BID}"
    #cp quality_mic1.rootrc quality.rootrc
else
    THEPATH="/eos/experiment/sndlhc/emulsionData/CERN/SND_mic${MIC}/RUN${RUN}/RUN${RUN}_W${WALL}_B${BID}"
    #cp quality_mics.rootrc quality.rootrc
fi
...
for (( i=FROMPLATE; i<=TOPLATE; i++ )); do
    if [ "$RESCAN" == "0" ]; then
        file=$(printf "%s/P%03d/tracks.raw.root" "$THEPATH" "$i")
	...
	new_destpath=$(printf "%s/RUN%i_W%i_B%i_P%03d" "$NEW_PLOTPATH" "$RUN" "$WALL" "$BID" "$i")
        ...
        file=$(printf "%s/P%03d_rescan/tracks.raw.root" "$THEPATH" "$i")
	...
	new_destpath=$(printf "%s/RUN%i_W%i_B%i_P%03d_rescan" "$NEW_PLOTPATH" "$RUN" "$WALL" "$BID" "$i")
	  ...
	  file=$(printf "%s/P%03d_rescan%d/tracks.raw.root" "$THEPATH" "$i" "$RESCAN")
	  ...
	  if [ ! -f "$file" ]; then
        ...
        file=$(printf "%s/P%03d_rescan%d/tracks.raw.root" "$THEPATH" "$i" "$RESCAN")
        destpath=$(printf "%s/b%06d/thickness/thickness_plate_%i_rescan%d" "$PLOTPATH" "$BRICK" "$i" "$RESCAN")
	new_destpath=$(printf "%s/RUN%i_W%i_B%i_P%03d_rescan%d" "$NEW_PLOTPATH" "$RUN" "$WALL" "$BID" "$i" "$RESCAN")
        if [ ! -f "$file" ]; then
          echo "${RED}++${NC} Skipping: $file does not exist ${RED}++${NC}"
          continue
        fi
